...
impl Display for NodeReorder {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        ...
        return writeln!(f, "Connections: ({}, {}), ", self.connection1, self.connection2);
    }
}
...
impl NodeReorder {
    pub fn new(position: usize,
        ...
        match node_type {
            ...
            NodeType::OutputNode => {
                connection1 = rand::thread_rng().gen_range(0..=nbr_inputs + graph_width - 1) as usize;
                connection2 = usize::MAX;
            }
        }
    ...
    fn mutate_connection(connection: &mut usize, upper_range: usize) {
        *connection = gen_random_number(*connection,
                                        upper_range);
    }
    ...
    fn mutate_computational_node(&mut self) {
        ...
        match rand_nbr {
            0 => NodeReorder::mutate_connection(&mut self.connection1,
                                                self.position),

            1 => NodeReorder::mutate_connection(&mut self.connection2,
                                                self.position),
        ...
        assert!(self.connection2 < self.position, "what was mutatet?: {}", rand_nbr);
    }
    ...
    pub fn set_new_position(&mut self, new_pos: usize, mutate_new_connections: bool) {
        if mutate_new_connections {
            ...
            if self.connection2 >= new_pos {
                NodeReorder::mutate_connection(&mut self.connection2,
                                               new_pos - 1);
            }
        }
