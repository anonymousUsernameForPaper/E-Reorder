...
impl Chromosome {
    ...
    pub fn evaluate(&mut self, inputs: &Array2<bool>, labels: &Array2<bool>) -> f32 {
        ...
        let output_end_id = (self.params.nbr_inputs + self.params.graph_width + self.params.nbr_outputs) as usize;
    ...
    fn swap_nodes(&mut self) {
        ...
        let comp_nodes_ids: Vec<usize> = (self.params.nbr_inputs..self.params.nbr_inputs + self.params.graph_width)
            .collect();
        let mut old_pos_inactive = vect_difference(&comp_nodes_ids, &self.active_nodes.as_ref().unwrap());
        ...
        assert_eq!(self.active_nodes.as_ref().unwrap().len(), new_pos_active.len());
        assert_eq!(old_pos_inactive.len(),
                   new_pos_inactive.len(),
                   "actives: \n{:?} \n{:?}", self.active_nodes.as_ref().unwrap(), new_pos_active);

        let mut swapped_pos_indices: HashMap<usize, usize, nohash_hasher::BuildNoHashHasher<usize>> = HashMap::default();
        swapped_pos_indices.reserve(self.params.nbr_inputs + self.params.graph_width + self.params.nbr_outputs);
        ...
        for (old_node_id, new_node_id) in self.active_nodes.as_ref().unwrap()
            .iter()
        ...
        for (old_node_id, new_node_id) in old_pos_inactive.iter().zip(new_pos_inactive.iter()) {
            ...
            assert!(new_nodes_grid[*new_node_id].position > new_nodes_grid[*new_node_id].connection1, "assert 2 for node: {}", *new_node_id);
            assert!(new_nodes_grid[*new_node_id].position > new_nodes_grid[*new_node_id].connection2, "assert 3 for node: {}", *new_node_id);
        }
        ...
        for node_id in (self.params.nbr_inputs + self.params.graph_width)..(self.params.nbr_inputs + self.params.graph_width + self.params.nbr_outputs) {
            Chromosome::update_connections(&mut new_nodes_grid, node_id, &mut swapped_pos_indices);
        }
    ...
    fn update_connections(new_nodes_grid: &mut Vec<NodeReorder>,
        ...
        new_nodes_grid[node_id].connection1 = *swapped_pos_indices.get(&con1)
            .unwrap_or_else(|| { &con1 });
        new_nodes_grid[node_id].connection2 = *swapped_pos_indices.get(&con2)
            .unwrap_or_else(|| { &con2 });
    }
}
